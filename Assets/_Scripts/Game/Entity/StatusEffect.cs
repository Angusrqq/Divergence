...
public class StatusEffect
{
    ...
    public StatusEffect(StatusType type, MonoBehaviour sender, string name, Enemy target, Action tickMethod = null,
        int timesApplied = 1, float delayBetweenTicks = 0f)
    {
        Type = type;
        _name = name;
        _sender = sender;
    ...
    public static StatusEffect operator +(StatusEffect left, StatusEffect right)
    {
        ...
        return new StatusEffect(left.Type, left._sender, left._name, left._target, method, left.TimesApplied + right.TimesApplied, left._delayBetweenTicks);
    }
    ...
    {
        if (Time.timeAsDouble - _lastTickTime >= _delayBetweenTicks)
        {
            _tickMethod();
...
public class NegativeStatusEffect : StatusEffect
{
    ...
    public NegativeStatusEffect(MonoBehaviour sender, string name, Enemy target, Action tickMethod = null)
    : base(StatusType.Negative, sender, name, target, tickMethod) { }

    protected override void TickMethod()
    {
        _target.TakeDamage(_sender.gameObject, _damage * _timesApplied, GetType());
    }
}

public class PositiveStatusEffect : StatusEffect // i feel stupid
{
    public PositiveStatusEffect(MonoBehaviour sender, string name, Enemy target, Action tickMethod = null)
    : base(StatusType.Positive, sender, name, target, tickMethod) { }
}